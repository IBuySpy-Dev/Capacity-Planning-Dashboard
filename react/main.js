...
function buildCapacityScoreSnapshotMessage(scoreRows, desiredCount) {
  ...
  if (!latestSnapshot) {
    return `No capacity snapshot available yet for desired count ${desiredCount}. Use Refresh Live Placement to calculate a live score.`;
  }
...
function App() {
  ...
  const [capacityGridStatus, setCapacityGridStatus] = useState({ loading: false, error: null });
  ...
  const [familySummaryError, setFamilySummaryError] = useState(null);
  ...
  useEffect(() => {
    ...
    async function loadCapacityGrid() {
      setCapacityGridStatus({ loading: true, error: null });
      try {
        ...
        setCapacityGridStatus({ loading: false, error: null });
      } catch (error) {
        if (capacityGridRequestRef.current !== requestId) {
          return;
        }
        ...
        setCapacityGridStatus({ loading: false, error: error.message || 'Failed to load capacity grid.' });
  ...
  useEffect(() => {
    ...
    async function loadAnalytics() {
      ...
      if (familyResult.status === 'fulfilled') {
        ...
        setFamilySummaryError(null);
      } else {
        setFamilyRows([]);
        setFamilySummaryError(familyResult.reason?.message || 'Failed to retrieve family summary.');
      }
