...
.PARAMETER EntraClientSecretKvUri
Optional Key Vault secret URI for the Entra client secret. When provided,
bootstrap stores it as the ENTRA_CLIENT_SECRET_KEYVAULT_SECRET_URI GitHub
repository variable for downstream Bicep deployments.
...
if ($EntraClientSecretKvUri) {
    Write-Host "Setting ENTRA_CLIENT_SECRET_KEYVAULT_SECRET_URI variable..."
    gh variable set ENTRA_CLIENT_SECRET_KEYVAULT_SECRET_URI `
        --body $EntraClientSecretKvUri `
        --repo "$GitHubOrganization/$GitHubRepository"
    Write-Host "✅ ENTRA_CLIENT_SECRET_KEYVAULT_SECRET_URI set"
}
